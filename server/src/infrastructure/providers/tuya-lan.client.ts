...
export class TuyaLanClient {
  ...
  async queryStatus(cid?: string | null): Promise<JsonObject> {
    const message = await this.sendAndReceiveNonEmpty(this.encodeMessage(this.nextSequence(), CMD_DP_QUERY_NEW, this.queryPayload(cid)), true);
    return this.decodePayload(message.payload);
  }

  async setDpsValue(dpsId: string, value: unknown, cid?: string | null): Promise<JsonObject> {
    const message = await this.sendAndReceiveNonEmpty(this.encodeMessage(this.nextSequence(), CMD_CONTROL_NEW, this.controlPayload(dpsId, value, cid)), true);
  ...
  async setDpsValueNowait(dpsId: string, value: unknown, cid?: string | null): Promise<JsonObject> {
    this.writeMessage(this.encodeMessage(this.nextSequence(), CMD_CONTROL_NEW, this.controlPayload(dpsId, value, cid)));
  ...
  private async negotiateSessionKey(): Promise<void> {
    ...
    const negotiationPayload = aesDecryptPadded(response.payload, this.realLocalKey);
    ...
    this.currentKey = aesEncryptNoPad(xorNonce, this.realLocalKey);
  }
  ...
  private decodePayload(payload: Buffer): JsonObject {
    ...
    let plainPayload = aesDecryptPadded(payload, this.currentKey);
    if (plainPayload.subarray(0, VERSION_34_HEADER.length).equals(VERSION_34_HEADER)) {
      plainPayload = plainPayload.subarray(VERSION_34_HEADER.length);
    }
