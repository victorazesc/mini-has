...
export class CommandsService {
  ...
  private async executeTuyaCommand(device: Device, secrets: JsonObject, request: CommandRequest): Promise<CommandResult> {
    ...
    try {
      ...
      const relinkedDevice = this.autoLinkLocalDevice(device.id);
  ...
  private async executeSmartthingsCommand(device: Device, request: CommandRequest): Promise<CommandResult> {
    ...
    let integration = this.getIntegration(device.integrationId);
    const latestIntegration = this.findLatestIntegrationByType('smartthings_cloud');
  ...
  private autoLinkLocalDevice(deviceId: number): Device | null {
    try {
      return this.moduleRef.get<DeviceLinker>('DEVICE_SERVICE', { strict: false }).autoLinkLocalDevice(deviceId);
    } catch {
      return null;
    }
  }

  private getIntegration(integrationId: number): StoredIntegration | null {
  ...
  private findLatestIntegrationByType(providerType: string): StoredIntegration | null {
