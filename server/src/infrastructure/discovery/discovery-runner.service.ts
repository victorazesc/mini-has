...
import { CreateDiscoveryJobRequest, Device, DiscoveredDevice, DiscoveredService, JobStatus, JsonObject, ProbeMode, DiscoveryJob, SavedDiscoveryDevice, SavedDiscoveryScan } from '../../types';
...
type InboxWriter = {
  upsertInboxItem(sourceType: string, sourceId: number, externalId: string, payload: JsonObject, secrets?: JsonObject, matchScore?: number): number;
};
...
export class DiscoveryService {
  ...
  createDiscoveryJob(rawRequest: CreateDiscoveryJobRequest): DiscoveryJob {
    ...
    const job: DiscoveryJob = { id: String(scanId), status: 'pending', progress: 0, result: [], created_at: now };
  ...
  async runDiscoveryJob(jobId: string, rawRequest: CreateDiscoveryJobRequest): Promise<void> {
    const request = normalizeRequest(rawRequest);
    this.updateJob(jobId, { status: 'running', started_at: this.storage.utcNow(), progress: 0.05 });
    try {
      const result = await this.runDiscovery(request);
      this.updateJob(jobId, { status: 'finished', progress: 1, result, finished_at: this.storage.utcNow() });
    } catch (error) {
      this.updateJob(jobId, { status: 'failed', error: messageFrom(error), finished_at: this.storage.utcNow() });
    }
  }
  ...
  listSavedScans(limit = 100): SavedDiscoveryScan[] {
    return this.storage.all<JsonObject>('SELECT * FROM discovery_scans ORDER BY id DESC LIMIT ?', [limit]).map((row) => this.scanFromRow(row));
  }
  ...
  private async runDiscovery(request: Required<CreateDiscoveryJobRequest>): Promise<DiscoveredDevice[]> {
    ...
    let devices = filterValidDevices(mergeResults(await readArpTable()), request.subnet_prefix);
    if (request.scan_ports) {
      const ips = devices.map((device) => device.ip).filter(Boolean) as string[];
      const portsByIp = await scanOpenPorts(ips, request.ports || DEFAULT_PORTS);
  ...
  private updateJob(jobId: string, updates: Partial<DiscoveryJob>): DiscoveryJob | null {
    ...
    if (scanId !== null) this.updateScanRecord(scanId, updates);
  ...
  private upsertDiscoveryInbox(scanId: number, key: string, device: DiscoveredDevice): void {
    const payload = { ...stripRaw(device), externalId: key, provider: 'discovery', localDeviceKey: key, scanId };
    this.moduleRef.get<InboxWriter>('INBOX_SERVICE', { strict: false }).upsertInboxItem('discovery', 0, key, payload, {}, device.confidence);
  }
...
async function pingSweep(subnetPrefix: string, timeoutSeconds: number): Promise<void> {
  ...
  await runLimited(ips, 64, async (ip) => {
    const args = process.platform === 'darwin' ? ['-c', '1', '-W', String(Math.floor(timeoutSeconds * 1000)), ip] : ['-c', '1', '-W', String(Math.max(1, Math.floor(timeoutSeconds))), ip];
    try {
      await execFileAsync('ping', args, { timeout: Math.floor((timeoutSeconds + 0.5) * 1000) });
    } catch {
...
async function readArpTable(): Promise<JsonObject[]> {
  try {
    ...
    return stdout
      .split('\n')
      .map((line) => {
        const ip = line.match(/\((\d+\.\d+\.\d+\.\d+)\)/)?.[1] || line.match(/^(\d+\.\d+\.\d+\.\d+)\s/)?.[1];
        const mac = line.match(/([0-9a-fA-F]{1,2}(?::[0-9a-fA-F]{1,2}){5})/)?.[1];
        if (!ip || !mac) return null;
        return { ip, mac: mac.split(':').map((part) => part.padStart(2, '0')).join(':').toUpperCase(), source: ['arp'] };
      })
...
async function scanOpenPorts(ips: string[], ports: number[], timeout = 400): Promise<Map<string, number[]>> {
  const result = new Map<string, number[]>(ips.map((ip) => [ip, []]));
  await runLimited(
    ips.flatMap((ip) => ports.map((port) => ({ ip, port }))),
    256,
    async ({ ip, port }) => {
      if (await canConnect(ip, port, timeout)) result.get(ip)?.push(port);
    },
  );
  for (const [ip, openPorts] of result) result.set(ip, openPorts.sort((a, b) => a - b));
  return result;
}
...
async function probeDevices(devices: DiscoveredDevice[], probeMode: ProbeMode): Promise<DiscoveredDevice[]> {
  if (probeMode === 'light') {
    return Promise.all(devices.map(withReverseDns));
  }
  return runLimited(devices, probeMode === 'aggressive' ? 64 : 24, (device) => probeDevice(device, probeMode));
}

async function probeDevice(device: DiscoveredDevice, probeMode: ProbeMode): Promise<DiscoveredDevice> {
  ...
  let probed = false;
  if (hostname && !device.hostname) updates.hostname = hostname;

  let httpPorts = device.openPorts.filter((port) => HTTP_PORTS.has(port) || HTTPS_PORTS.has(port)).sort((a, b) => a - b);
  if (probeMode === 'balanced') httpPorts = httpPorts.filter((port) => [80, 443, 8080, 8123, 8266].includes(port));
  for (const result of await Promise.all(httpPorts.map((port) => probeHttp(device.ip as string, port)))) {
    if (!result) continue;
    probed = true;
    raw.probes = [...(raw.probes || []), result];
    services.push(serviceFromHttp(result));
    mergeProbeIdentity(updates, result);
  }

  for (const result of await Promise.all(device.openPorts.filter((port) => RTSP_PORTS.has(port)).map((port) => probeRtsp(device.ip as string, port)))) {
    if (!result) continue;
    probed = true;
    raw.probes = [...(raw.probes || []), result];
    services.push({ type: 'rtsp', port: result.port, properties: { server: result.server, public: result.public } });
  }

  const bannerPorts = device.openPorts.filter((port) => [21, 22, 23, 1883, 6053, 8266, 8883].includes(port) && (probeMode === 'aggressive' || [22, 6053, 8266].includes(port)));
  ...
  if (probed) updates.source = [...new Set([...device.source, 'probe'])].sort();
  if (Object.keys(raw).length) updates.raw = raw;
  if (services.length) updates.services = dedupeServices(services);
...
async function probeRtsp(ip: string, port: number): Promise<JsonObject | null> {
  const response = await tcpExchange(ip, port, `OPTIONS rtsp://${ip}:${port}/ RTSP/1.0\r\nCSeq: 1\r\nUser-Agent: mini-has/1.0\r\n\r\n`, 1200);
  if (!response || !response.includes('RTSP/')) return null;
  const headers = parseHeaderLines(response.split('\n').slice(1));
  return { type: 'rtsp', port, server: headers.server, public: headers.public };
}
...
async function probeBanner(ip: string, port: number): Promise<JsonObject | null> {
  ...
  return { type: serviceTypeForPort(port), port, banner };
}

function tcpExchange(ip: string, port: number, request: string, timeoutMs: number): Promise<string | null> {
  return new Promise((resolve) => {
    const socket = new Socket();
    const chunks: Buffer[] = [];
...
function toDevice(item: JsonObject): DiscoveredDevice {
  return {
    ...
    openPorts: item.openPorts || item.open_ports || [],
...
function findExistingKey(devices: Map<string, DiscoveredDevice>, device: DiscoveredDevice): string | null {
  for (const [key, current] of devices) {
    if (device.mac && current.mac && device.mac.toUpperCase() === current.mac.toUpperCase()) return key;
    if (device.ip && current.ip && device.ip === current.ip) return key;
    if (device.hostname && current.hostname && device.hostname.toLowerCase() === current.hostname.toLowerCase()) return key;
  }
  return null;
}
...
function isValidDevice(device: DiscoveredDevice, subnetPrefix: string): boolean {
  ...
  return subnetPrefix.includes('/') ? cidrContains(subnetPrefix, device.ip) : device.ip.startsWith(`${subnetPrefix.replace(/\.$/, '')}.`);
}
...
function enrichDevice(device: DiscoveredDevice): DiscoveredDevice {
  ...
  const deviceType = device.deviceType || inferDeviceType(device, manufacturer);
...
function serviceFromHttp(result: JsonObject): DiscoveredService {
  return { type: result.scheme, port: result.port, name: result.title || result.server || result.realm, properties: result };
}

function serviceFromBanner(result: JsonObject): DiscoveredService {
  return { type: result.type, port: result.port, properties: result };
}

function mergeProbeIdentity(updates: Partial<DiscoveredDevice>, result: JsonObject): void {
  if (!updates.name && result.title) updates.name = String(result.title).slice(0, 80);
  if (!updates.model && result.model) updates.model = String(result.model).slice(0, 120);
}
...
function parseHttpHeaders(text: string): Record<string, string> {
  ...
  if (parts.length >= 2) headers[':status'] = parts[1];
  return { ...headers, ...parseHeaderLines(lines.slice(1)) };
}
...
function dedupeServices(services: DiscoveredService[]): DiscoveredService[] {
  ...
  for (const service of services) deduped.set(`${service.type || ''}:${service.port || ''}:${service.name || ''}`, service);
...
function subnetIps(subnetPrefix: string): string[] {
  if (!subnetPrefix.includes('/')) return Array.from({ length: 254 }, (_, index) => `${subnetPrefix.replace(/\.$/, '')}.${index + 1}`);
  ...
  const ips: string[] = [];
  for (let value = network + 1; value < broadcast; value += 1) ips.push(intToIp(value));
  return ips;
}
...
function emptyDiscoveredDevice(): DiscoveredDevice {
  return { source: [], services: [], openPorts: [], confidence: 0 };
}
