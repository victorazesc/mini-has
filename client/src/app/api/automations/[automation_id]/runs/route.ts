...
export async function GET(
    ...
    try {
        const response = await fetch(`${env.SERVER_URL}/automations/${automation_id}/runs?limit=${limit}`, {
        ...
        if (!response.ok) {
            return NextResponse.json(
                { message: data?.detail ?? data?.message ?? "Falha ao buscar execuções da automação." },
